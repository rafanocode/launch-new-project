...
## Hard rules
- One confirmation gate only. Preflight is the sole earlier stop for tool/credential availability; Phase 1's Supabase org-count check (item 7) may also stop early, but only for the read-only precondition "this account has zero organizations" — it never asks a question or creates anything. Never log in on the user's behalf. Never print or write secret values. On failure, stop + report + offer cleanup; never leave half-created resources silently.
